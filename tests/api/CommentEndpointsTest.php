...
{ 
    ...
    */
    public function setUp() {
        ...
        $this->comment = new Tasteez\Models\Comment(self::$conn);
    ...
    public function testAddCommentApiBody() {
        echo "Running: Get body of add comment API endpoint\n";
        ...
        $this->data = json_encode(array("comment" => "test", "meal_id" => 52820));
        $this->url  = "http://localhost:8080/api/meal/52820/add-comment"; 
        curl_setopt($this->curl, CURLOPT_POSTFIELDS, $this->data);
        curl_setopt($this->curl, CURLOPT_HTTPHEADER, array('Content-Type:application/json'));
        curl_setopt($this->curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($this->curl, CURLOPT_URL, $this->url );
    ...
    public function testAddCommentApiStatus(){
        echo "Running: Get status of add comment API endpoint\n";
        $this->data = json_encode(array("username" => "test", "password" => "test"));
        $this->url = "http://localhost:8080/api/auth/login";
        $this->curl = curl_init();  
        curl_setopt($this->curl, CURLOPT_POSTFIELDS, $this->data);
    ...
    public function testDeleteCommentApiBody() {
        echo "Running: Get body of delete comment API endpoint\n";
        $this->data = json_encode(array("username" => "test", "password" => "test"));
        $this->url = "http://localhost:8080/api/auth/login";
        $this->curl = curl_init();  
        curl_setopt($this->curl, CURLOPT_POSTFIELDS, $this->data);
        ...
        curl_setopt($this->curl, CURLOPT_URL, $this->url);
        $this->body = curl_exec($this->curl);
        $this->data = json_encode(array("comment" => "test"));
        $this->url  = "http://localhost:8080/api/meal/52820/add-comment"; 
        curl_setopt($this->curl, CURLOPT_POSTFIELDS, $this->data);
        curl_setopt($this->curl, CURLOPT_HTTPHEADER, array('Content-Type:application/json'));
        curl_setopt($this->curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($this->curl, CURLOPT_URL, $this->url );
        $this->body = curl_exec($this->curl);
        $comment = $this->comment->getAll(52820, $this->id);
        $this->comment_id = $comment[0]["comment_id"];
        $this->data = json_encode(array("comment_id" => (int) $this->comment_id));
        $this->url  = "http://localhost:8080/api/meal/52820/delete-comment"; 
        curl_setopt($this->curl, CURLOPT_POSTFIELDS, $this->data);
    ...
    public function testDeleteCommentApiStatus(){
        echo "Running: Get status of delete comment API endpoint\n";
        $this->data = json_encode(array("username" => "test", "password" => "test"));
        $this->url = "http://localhost:8080/api/auth/login";
        $this->curl = curl_init();  
        curl_setopt($this->curl, CURLOPT_POSTFIELDS, $this->data);
    ...
    */
    public function tearDown() {
        ...
        $this->comment->deleteComment((int) $this->comment_id, $this->id);
        $this->curl = null;
        $this->body = null;
        $this->testUser = null;
        $this->url = "";
